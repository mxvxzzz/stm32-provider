...
build_cmd() {
  ...
  algo="$2"
  case "$IMPL" in
    soft|pv_afalg|pv_cryptodev|eng_afalg|eng_cryptodev)
      echo "taskset -c 0 ./evp_bench $IMPL $SECS $bytes $algo"
      ;;
    *)
...
run_algo() {
  ...
  sh -c "$CMD" > "$tmp_speed" 2>/dev/null || true
...
for BYTES in $BLOCKS; do
  ...
  for algo in $algos; do
    ajson=$(run_algo "$BYTES" "$algo")
    if [ "$first_algo" -eq 1 ]; then
      algo_jsons="$ajson"
      first_algo=0
    else
      algo_jsons="$algo_jsons,$ajson"
    fi
    sleep 1
  done
